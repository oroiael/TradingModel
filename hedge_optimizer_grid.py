...
class HedgeOptimizerGrid:
    def __init__(self, options_data_path, ibkr_5min_path, initial_capital=150000):
        ...
        print("Proxy Engine Ready: Hedge Optimizer Initiated.\n")

    def simulate_hedge(self, max_debit, sell_delta_target, ratio_buy_qty):
        # LOCKED INCOME ENGINE PARAMETERS
        dte_range = (30, 60)
        target_width = 5.0
        min_credit = 0.85
        alloc_pct = 0.15
        ...
        for current_date in trading_days:
            ...
            for trade in open_trades:
                ...
                is_closed, net_pnl_per_contract = False, 0.0
                ...
                if current_underlying_low <= trade['Short Strike']:
                    ...
                    prb_value_at_crash = (ratio_buy_qty * bs_hedge_buy) - bs_hedge_sell
                    hedge_profit = prb_value_at_crash - trade['PRB Entry Net Cost']
                    
                    net_loss_raw = gross_spread_loss - hedge_profit
                    total_legs = 2 + 1 + ratio_buy_qty # Short, Long, Hedge Sell, Hedge Buys
                    net_pnl_per_contract = -(net_loss_raw + (slippage_per_leg * total_legs)) * 100 
                ...
                # B. NORMAL TAKE PROFIT / EXPIRATION
                    ...
                    prb_eod_value = (ratio_buy_qty * curr_hedge_buy) - curr_hedge_sell
                    hedge_pnl = prb_eod_value - trade['PRB Entry Net Cost']
                    
                    total_legs = 2 + 1 + ratio_buy_qty
                    net_pnl_per_contract = (gross_pnl + hedge_pnl - (slippage_per_leg * total_legs)) * 100
                    is_closed = True
                ...
                if is_closed:
                    ...
                    closed_trades.append({'Net PnL': total_combined_pnl, 'Drawdown (%)': drawdown_pct})
                else:
                    still_open.append(trade)
            ...
            if len(open_trades) < max_trades:
                ...
                if (net_credit_raw * 100) >= (min_credit * 100):
                    # --- CONSTRUCT OPTIMIZED PRB HEDGE ---
                    # 1. Target Sell Delta
                    prb_sell_cands = daily_puts[(daily_puts['Expiration'] == short_put['Expiration'])]
                    if prb_sell_cands.empty: continue
                    prb_sell = prb_sell_cands.iloc[(prb_sell_cands['delta'] - sell_delta_target).abs().argsort()].iloc[0]
                    
                    # 2. Find Buy Strikes that satisfy the Max Debit allowance
                    target_buy_cost_max = (prb_sell['close'] + max_debit) / ratio_buy_qty
                    ...
                    # Pick the highest possible strike that fits the budget (minimizes Valley of Death)
                    prb_buy = prb_buy_cands.sort_values(by='strike', ascending=False).iloc[0]
                    prb_net_cost = (ratio_buy_qty * prb_buy['close']) - prb_sell['close']
                    
                    total_legs = 2 + 1 + ratio_buy_qty
                    net_credit_realized = net_credit_raw - prb_net_cost - (slippage_per_leg * total_legs)
                    
                    # Only take the trade if we still make at least $0.20 after insurance and slippage
        ...
        if pnl_df.empty: return 0, 0, 0, 0
        
        wins = len(pnl_df[pnl_df['Net PnL'] > 0])
        win_rate = (wins / len(pnl_df)) * 100
        roi = ((trading_balance + swept_cash - self.initial_capital) / self.initial_capital) * 100
        return len(pnl_df), win_rate, roi, pnl_df['Drawdown (%)'].max()

    def run_grid(self):
        debits = [0.00, 0.15, 0.30, 0.50]
        ratios = [2, 3, 4]  # 1x2, 1x3, 1x4
        deltas = [-0.10, -0.15, -0.20]
        
        total_runs = len(debits) * len(ratios) * len(deltas)
        print(f"Testing {total_runs} PRB Hedge Configurations...\n")
        
        results = []
        count = 0
        
        for debit, ratio, delta in itertools.product(debits, ratios, deltas):
            count += 1
            trades, win_rate, roi, max_dd = self.simulate_hedge(max_debit=debit, sell_delta_target=delta, ratio_buy_qty=ratio)
            
            if trades > 50:
                results.append({
                    'Max Debit Paid': f"${debit:.2f}",
                    'Hedge Start Delta': f"{int(abs(delta)*100)}",
                    'PRB Ratio': f"1 x {ratio}",
                    'Total Trades': trades,
        ...
        res_df = pd.DataFrame(results).sort_values(by='Total ROI %', ascending=False)
        res_df.to_csv("SOXL_HEDGE_OPTIMIZER_GRID.csv", index=False)
        
        print("\n=== TOP 10 OPTIMIZED HEDGE CONFIGURATIONS ===")
        print(res_df.head(10).to_string(index=False))

if __name__ == "__main__":
    env = HedgeOptimizerGrid("SOXL_Master_Cleaned.csv", "SOXL_5min_3Years.csv", 150000)
    env.run_grid()
