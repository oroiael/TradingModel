...
def norm_cdf(x):
    """Standard Normal Cumulative Distribution Function without needing SciPy"""
    return (1.0 + math.erf(x / math.sqrt(2.0))) / 2.0

def black_scholes_put(S, K, T, r, sigma):
    """Calculates Theoretical Put Price using Black-Scholes."""
...
class RatioHedgeProxySimulator:
    def __init__(self, options_data_path, ibkr_5min_path, initial_capital=150000):
        ...
        # Cache both Close Price and Implied Volatility
        self.options_cache = self.put_df.set_index(['Date', 'Expiration', 'strike'])['close'].to_dict()
        
        # Broader search for ThetaData's IV column naming conventions
        iv_cols_to_check = ['iv', 'implied_volatility', 'implied_vol', 'volatility', 'implied_volatility_1545']
        iv_col = next((col for col in self.put_df.columns if col.lower() in iv_cols_to_check), None)
        
        if iv_col:
            self.iv_cache = self.put_df.set_index(['Date', 'Expiration', 'strike'])[iv_col].to_dict()
        else:
            print("WARNING: Could not find IV column in options data. Black-Scholes will default to 0.80 IV.")
            self.iv_cache = {}
        ...
        # --- PROCESS THE 5-MINUTE IBKR DATA ---
        ...
        # FIX: IBKR outputs dates as "20230707 09:30:00 America/New_York". 
        # We slice the first 8 characters to extract just "20230707" for a clean Flat Date conversion.
        ibkr_df['FlatDate'] = pd.to_datetime(ibkr_df[date_col].astype(str).str[:8], format='%Y%m%d')
        
        # Group the 5-min bars to find the absolute lowest print of each trading day
        self.daily_lows = ibkr_df.groupby('FlatDate')[low_col].min().to_dict()
        
        print("Proxy Engine Ready: Black-Scholes Vega-Shock modeling enabled.\n")

    def run_simulation(self):
        # OPTIMIZED INCOME ENGINE (From Grid Results)
        dte_range = (30, 60)
        target_width = 5.0
        min_credit = 0.85
        alloc_pct = 0.15
        ...
        # RATIO HEDGE PARAMETERS & REALITY TAXES
        risk_free_rate = 0.05
        vega_shock_multiplier = 1.30  # Intraday crash spikes IV by 30%
        slippage_per_leg = 0.05       # Slippage cost per contract
        base_sweep_pct = 0.10
        max_contracts = 300
                       
        print("--- EXECUTING PUT RATIO BACKSPREAD (PRB) SIMULATION ---")
        ...
        for current_date in trading_days:
            ...
            # Use exact 5-min low, fallback to EOD price if missing
            ...
            for trade in open_trades:
                ...
                # Fetch Real EOD prices for Income Engine
                ...
                is_closed, res_reason, net_pnl_per_contract = False, "", 0.0
                
                # A. INTRADAY TRIPWIRE (Flash Crash = Hedge Activation)
                if current_underlying_low <= trade['Short Strike']:
                    # 1. Income Engine takes the exact 3.0x Max Loss
                    gross_spread_loss = (trade['Entry Net Credit'] * stop_loss_mult) - trade['Entry Net Credit']
                    
                    # 2. Black-Scholes dynamically calculates the PRB Hedge value at the moment of the crash
                    dte_remaining = max(1, (trade['Expiration'] - current_date).days)
                    t_years = dte_remaining / 365.0
                    
                    # Apply 30% Panic IV Shock to the original entry IV
                    ...
                    # PRB Value: 2 Longs - 1 Short
                    prb_value_at_crash = (2 * bs_hedge_buy) - bs_hedge_sell
                    hedge_profit = prb_value_at_crash - trade['PRB Entry Net Cost']
                    
                    # Combine Losses + Payouts + Slippage on all legs
                    net_loss_raw = gross_spread_loss - hedge_profit
                    net_pnl_per_contract = -(net_loss_raw + (slippage_per_leg * 5)) * 100 # 2 main legs + 3 hedge legs
                    
                    # Cap loss at theoretical max risk
                    net_pnl_per_contract = max(net_pnl_per_contract, -trade['Base Risk'])
                    is_closed = True
                    res_reason = "CRASH + BS PRB PAYOUT"
                ...
                # B. NORMAL TAKE PROFIT / EXPIRATION
                    ...
                    # EOD Hedge Prices for quiet days
                    ...
                    prb_eod_value = (2 * curr_hedge_buy) - curr_hedge_sell
                    hedge_pnl = prb_eod_value - trade['PRB Entry Net Cost']
                    
                    net_pnl_per_contract = (gross_pnl + hedge_pnl - (slippage_per_leg * 5)) * 100
                    is_closed = True
                    res_reason = "EXPIRATION" if current_date >= trade['Expiration'] else f"TAKE-PROFIT ({int(take_profit_pct*100)}%)"
            ...
            if len(open_trades) < max_trades:
                ...
                if (net_credit_raw * 100) >= (min_credit * 100):
                    # --- CONSTRUCT THE PUT RATIO BACKSPREAD HEDGE ---
                    # 1. Find a Put to sell (e.g., Delta ~0.15)
                    prb_sell_cands = daily_puts[(daily_puts['Expiration'] == short_put['Expiration']) & (daily_puts['delta'] > -0.20) & (daily_puts['delta'] < -0.10)]
                    if prb_sell_cands.empty: continue
                    prb_sell = prb_sell_cands.iloc[0]
                    
                    # 2. Find a Put to Buy 2x for roughly half the price (Net Zero Cost)
                    target_buy_price = prb_sell['close'] / 2.0
                    prb_buy_cands = daily_puts[(daily_puts['Expiration'] == short_put['Expiration']) & (daily_puts['strike'] < prb_sell['strike'])]
                    if prb_buy_cands.empty: continue
                    prb_buy = prb_buy_cands.iloc[(prb_buy_cands['close'] - target_buy_price).abs().argsort()].iloc[0]
                    
                    prb_net_cost = (2 * prb_buy['close']) - prb_sell['close']
                    
                    # Only accept the trade if the hedge is free or generates a tiny credit
                    if prb_net_cost <= 0.05:  
                        
                        net_credit_realized = net_credit_raw - prb_net_cost - (slippage_per_leg * 5)
        ...
        pnl_df.to_csv('SOXL_PRB_Hedged_Audit.csv', index=False)
        
        wins = len(pnl_df[pnl_df['Net PnL'] > 0])
        tripwires = len(pnl_df[pnl_df['Reason'] == "CRASH + BS PRB PAYOUT"])
        win_rate = (wins / len(pnl_df)) * 100
        roi = ((pnl_df['Total Net Worth'].iloc[-1] - self.initial_capital) / self.initial_capital) * 100
        
        print(f"\n--- PRB BLACK-SCHOLES SIMULATION COMPLETE ({round(time.time() - start_time, 1)} sec) ---")
        print(f"Total Trades:          {len(pnl_df)}")
        print(f"Intraday Crashes:      {tripwires} (PRB Exploded via Vega Shock)")
        ...
        print("-" * 60)
        print(f"TRUE MAX DRAWDOWN:     {pnl_df['Drawdown (%)'].max():.2f}%")
        print("-" * 60)

if __name__ == "__main__":
    env = RatioHedgeProxySimulator("SOXL_Master_Cleaned.csv", "SOXL_5min_3Years.csv", 150000)
    env.run_simulation()