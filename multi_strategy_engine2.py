...
# ==============================================================================
# STRATEGY 3: SLOW REGIME (CASH) + FAST OPTIONS (WEEKLIES)
# ==============================================================================
SYMBOL = "SOXL"
STARTING_CAPITAL = 100000.0  

# THE SLOW FILTER: Keeps you safe from crashes
TREND_SMA_WINDOW = 50 
RISK_FREE_RATE = 0.05 / 252  

# THE FAST ENGINE: High-frequency income generation
CALL_TARGET_DELTA = 0.30   
CALL_FALLBACK_MULT = 1.02  # THE FIX: If Delta is missing, target 5% OTM (simulates 0.25 Delta)
CALL_DTE_MIN = 0           # THE FIX: Lowered to 2 so we can trade Mon/Tue/Wed for Friday expiry
CALL_DTE_MAX = 7  
CALL_TAKE_PROFIT = 0.20   
...
def run_goldilocks_strategy():
    print(f"Loading data for Strategy 3 (Slow Regime + Fast Options)...")
    df = load_data()
    ...
    for current_date in all_dates:
        ...
        if not is_uptrend:
            ...
            if active_call:
                ...
                active_call['Exit_Reason'] = "Regime Switch (Bailout to Cash)"
            ...
            # --- BULL REGIME: BUY SOXL & WRITE WEEKLIES ---
            if active_call:
                call_market = today_data[(today_data['Expiration'].dt.date == active_call['Expiration']) & 
                                         (today_data['strike'] == active_call['Strike']) & 
                                         (today_data['right'].str.startswith('C'))]
            ...
            if not active_call and shares > 0:
                ...
                if not valid_calls.empty:
                    ...
                    if 'delta' in valid_calls.columns and valid_calls['delta'].notna().any() and not (valid_calls['delta'] == 0).all():
                        ...
                        target_strike = underlying_price * CALL_FALLBACK_MULT
    ...
    pd.DataFrame(portfolio_log).to_csv("raw_data/V4_Goldilocks_Portfolio.csv", index=False)
    trade_df = pd.DataFrame(trade_log) if trade_log else pd.DataFrame()
    if not trade_df.empty:
        trade_df.to_csv("raw_data/V4_Goldilocks_Trades.csv", index=False)
    
    print("\n========================================")
    print("STRATEGY 3: SLOW REGIME + FAST OPTIONS")
    print("========================================")
